...
The fuel zone is homogenized from subassemblies consistent with AP1000 PWR
spent UO$_2$ fuel (60 GWd/MTU, 73 yr cooled):
    30 vol%  Spent UO$_2$ ceramic fuel (from CSV depletion inventory, 10.48 g/cm³)
    20 vol%  ZIRLO cladding (6.56 g/cm³)
...
# Configuration
# ==============================================================================
CSV_PATH = "uo2_bol_inventory_zpinch.csv"     # Path to spent fuel inventory CSV
...
# --- Spent fuel density — UO$_2$ ceramic at 95.5% theoretical density ---
# (10.97 g/cm³ × 0.955 = 10.48 g/cm³, post-irradiation swelling negligible)
SPENT_FUEL_DENSITY = 10.48                         # g/cm³ (AP1000 UO$_2$ pellet)

# --- ZIRLO cladding (Zr-1.0Nb-1.0Sn-0.1Fe) ---
ZIRLO_DENSITY = 6.56                               # g/cm³

# --- Fuel zone volume fractions ---
FUEL_VOL_FRAC    = 0.30                            # 30% UO$_2$ fuel pellets
CLAD_VOL_FRAC    = 0.20                            # 20% ZIRLO cladding + structural
COOLANT_VOL_FRAC = 0.50                            # 50% PbLi coolant

OUTPUT_DIR = f"uo2_zpinch_results_shell{f'{FUEL_SHELL_THICKNESS:.0f}'.zfill(2)}cm"
...
def parse_inventory(csv_path):
    ...
    counterparts.  Noble gases (He, Kr, Xe) are skipped — they escape or
    are released when spent fuel is repackaged for the blanket.
    """
    inventory = {}
...
def build_spent_fuel_material(inventory):
    ...
    fuel = openmc.Material(name="Spent_Fuel_UO2")
...
# ==============================================================================
# Build ZIRLO cladding material
# ==============================================================================

def build_zirlo_material():
    """
    Build ZIRLO cladding material (Zr-1.0Nb-1.0Sn-0.1Fe).
    Composition from Westinghouse AP1000 specification.
    """
    zirlo = openmc.Material(name="ZIRLO_cladding")
    zirlo.add_element("Zr", 0.979, percent_type="wo")
    zirlo.add_element("Nb", 0.010, percent_type="wo")
    zirlo.add_element("Sn", 0.010, percent_type="wo")
    zirlo.add_element("Fe", 0.001, percent_type="wo")
    zirlo.set_density("g/cm3", ZIRLO_DENSITY)
    return zirlo
...
def build_fuel_zone_homogenized(fuel_mat, clad_mat, pbli_mat):
    """
    Homogenize spent fuel pins into a single material.

    Volume fractions (spent UO$_2$ pins in PbLi blanket):
        Fuel pellets:   FUEL_VOL_FRAC    (UO$_2$ at 95.5% TD)
        ZIRLO:          CLAD_VOL_FRAC    (cladding + structural)
        PbLi coolant:   COOLANT_VOL_FRAC  (flowing through & around assemblies)
    """
    fuel_zone = openmc.Material.mix_materials(
        [fuel_mat, clad_mat, pbli_mat],
...
def run_case(shell_midpoint, pbli_mat, fuel_mat, run_index):
    ...
    fuel_mat : openmc.Material
        The homogenized fuel zone material (30 vol% spent UO$_2$ +
        20 vol% ZIRLO cladding + 50 vol% PbLi coolant).
    run_index : int
    ...
    if no_fuel:
        shell_offset = None
        case_dir = os.path.join(OUTPUT_DIR, f"uo2_case_{run_index:03d}_no_fuel")
    else:
        shell_offset = shell_midpoint - FUEL_SHELL_THICKNESS / 2.0
        case_dir = os.path.join(
            OUTPUT_DIR,
            f"uo2_case_{run_index:03d}_mid_{shell_midpoint:.1f}cm"
        )
    ...
    MESH_SCORES = [
        ...
        "heating-local", # 6. Energy deposition [eV/src]
        "fission-q-recoverable", # 6b. Recoverable fission energy deposition [eV/src]
    ...
    try:
        ...
        mesh_csv_path = "uo2_mesh_depth_profiles.csv"
        ...
        nbal_csv_path = "uo2_neutron_balance_depth.csv"
...
def plot_results(all_results):
    ...
    fig.suptitle(
        "Z-Pinch Cylindrical Wedge: UO$_2$ Fuel Shell Position Sensitivity\n"
    ...
    ax.set_title("(b) Fission Rate in Spent UO$_2$ Fuel")
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title("(c) Radiative Capture in Spent UO$_2$ Fuel")
    ax.grid(True, alpha=0.3)
    ...
    plot_path = os.path.join(OUTPUT_DIR, "uo2_zpinch_sensitivity.png")
    ...
    if fuel_only:
        ...
        ax2.set_title(
            "Neutron Spectrum in UO$_2$ Fuel Shell vs. Radial Position")
        ax2.legend(fontsize=9)
        ax2.grid(True, alpha=0.3, which="both")

        spec_path = os.path.join(
            OUTPUT_DIR, "uo2_zpinch_fuel_spectra_vs_position.png")
...
def plot_mesh_profiles(all_results):
    ...
    fig.suptitle(
        "Radial Depth-Resolved Mesh Tallies \u2014 "
        "Z-Pinch Cylindrical Wedge (UO$_2$)",
    ...
    mesh_plot_path = os.path.join(
        OUTPUT_DIR, "uo2_zpinch_mesh_depth_profiles.png")
    ...
    if found_current:
        ...
        current_path = os.path.join(
            OUTPUT_DIR, "uo2_zpinch_mesh_r_current_profile.png")
...
def plot_neutron_balance(all_results):
    ...
    for ci in indices:
        ...
        ax.set_title(
            ...
            f"UO$_2$ fuel shell midpoint = {midpt:.0f} cm"
        )
        ...
        plot_path = os.path.join(
            OUTPUT_DIR, f"uo2_zpinch_nbal_mid{midpt:.0f}cm.png")
        fig.savefig(plot_path, dpi=200)
...
def plot_cumulative_neutron_balance(all_results):
    ...
    for ci in indices:
        ...
        plot_path = os.path.join(
            OUTPUT_DIR,
            f"uo2_zpinch_cumulative_nbal_mid{midpt:.0f}cm.pdf"
        )
        fig.savefig(plot_path, dpi=200)
...
def main():
    ...
    print("  Zap Energy SFS Z-Pinch: UO2 Fuel Shell in PbLi Blanket")
    print("  Fuel zone: 30% spent UO2 / 20% ZIRLO / 50% PbLi (by volume)")
    print("=" * 72)
    print(f"\n  Geometry:")
    ...
    print("Building ZIRLO cladding material (Zr-1Nb-1Sn-0.1Fe)...")
    clad_mat = build_zirlo_material()
    ...
    print(f"    Fuel pellets (UO2):  {FUEL_VOL_FRAC*100:.0f} vol%  "
          f"@ {SPENT_FUEL_DENSITY} g/cm\u00b3")
    print(f"    ZIRLO cladding:       {CLAD_VOL_FRAC*100:.0f} vol%  "
          f"@ {ZIRLO_DENSITY} g/cm\u00b3")
    print(f"    PbLi coolant:         {COOLANT_VOL_FRAC*100:.0f} vol%  "
          f"@ {PBLI_DENSITY} g/cm\u00b3")
    fuel_zone_mat = build_fuel_zone_homogenized(fuel_mat, clad_mat, pbli_mat)
    homog_density = (FUEL_VOL_FRAC * SPENT_FUEL_DENSITY
                     + CLAD_VOL_FRAC * ZIRLO_DENSITY
    ...
    json_path = os.path.join(OUTPUT_DIR, "uo2_zpinch_all_results.json")
