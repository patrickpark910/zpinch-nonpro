"""
OpenMC Decay-Only Calculation — Spent Fuel in Storage
=====================================================

Reads an end-of-life isotopic inventory (atoms) from a CSV and decays it
in-place for 365 days with no neutron flux — pure radioactive decay via
the Bateman equations solved by OpenMC's CRAM-48 solver.

This models spent fuel sitting in a pool or dry cask: no external
neutron source, no fission, just radioactive decay and daughter
in-growth.  Passing an *empty* MicroXS object to IndependentOperator
tells OpenMC to run decay-only (no neutron-induced reactions).
...
# --- Input inventory ---
INPUT_CSV = "uzr_eol_inventory_tracked.csv"
OUTPUT_CSV = "uzr_bol_inventory_zpinch.csv"

# --- Decay schedule ---
DECAY_DAYS    = 365.0       # total decay time [days]
NUM_STEPS     = 12          # number of time steps (~monthly)

# --- Depletion chain ---
# Pre-built chains available at https://openmc.org/depletion-chains/
# Uncomment and set if not already configured via environment variable:
openmc.config['chain_file'] = "/home/patri/openmc/data/chain_endfb81_fast.xml"
...
# --- Output directories ---
OPENMC_DIR  = "uzr_openmc"   # XMLs, H5s, all OpenMC working files
...
def run_decay(op):
    ...
    print("=" * 60)
    print("Spent-Fuel Decay Calculation (no neutron source)")
    print("=" * 60)
    print(f"  Input         : {INPUT_CSV}")
    print(f"  Decay time    : {DECAY_DAYS:.0f} days")
...
def post_process(nuclide_atoms):
    ...
    print(f"\n{'='*72}")
    print(f" Decay summary: {DECAY_DAYS:.0f} days in spent-fuel storage")
    print(f"{'='*72}")
    ...
    csv_full = os.path.join(RESULTS_DIR, "uzr_delta_inventory.csv")
    ...
    tracked = [
        ...
        # Short-lived / interesting FPs
        "Zr95", "Sr90", "Cs137", "Tc99",
    ]
