...
The fuel zone is homogenized from subassemblies consistent with AP1000
PWR spent UO₂ fuel (60 GWd/MTU, 73 yr cooled):
...
# Annular fuel zone volume fractions (spent fuel pins in flowing PbLi)
...
OUTPUT_DIR = f"uo2_sigma_pile_results_slab{f'{FUEL_SLAB_THICKNESS:.0f}'.zfill(2)}cm"
...
def parse_inventory(csv_path):
    ...
    Noble gases (He, Kr, Xe) are skipped — they escape or are released
    when spent fuel is repackaged for the blanket.
    """
    inventory = {}
    ...
    with open(csv_path, "r") as f:
        ...
        for row in reader:
            ...
            # Skip noble gases (released from spent fuel during repackaging)
...
def run_case(slab_midpoint, pbli_mat, fuel_mat, run_index):
    ...
    if no_fuel:
        ...
        case_dir = os.path.join(OUTPUT_DIR, f"uo2_case_{run_index:03d}_mid_{slab_midpoint:.1f}cm")
...
def plot_results(all_results):
    ...
    plot_path = os.path.join(OUTPUT_DIR, "uo2_sigma_pile_sensitivity.png")
    ...
    if fuel_only:
        ...
        spec_path = os.path.join(OUTPUT_DIR, "uo2_fuel_spectra_vs_position.png")
...
def plot_mesh_profiles(all_results):
    ...
    mesh_plot_path = os.path.join(OUTPUT_DIR, "uo2_mesh_depth_profiles.png")
    ...
    if found_current:
        ...
        current_path = os.path.join(OUTPUT_DIR, "uo2_mesh_z_current_profile.png")
...
def plot_neutron_balance(all_results):
    ...
    for ci in indices:
        ...
        plot_path = os.path.join(OUTPUT_DIR, f"uo2_neutron_balance_mid{midpt:.0f}cm.png")
        fig.savefig(plot_path, dpi=200)
...
def plot_cumulative_neutron_balance(all_results):
    ...
    for ci in indices:
        ...
        plot_path = os.path.join(OUTPUT_DIR, f"uo2_cumulative_nbal_mid{midpt:.0f}cm.png")
        fig.savefig(plot_path, dpi=200)
...
def main():
    ...
    print(f"    Fuel pellets (UO₂):   {FUEL_VOL_FRAC*100:.0f} vol%  @ {SPENT_FUEL_DENSITY} g/cm³")
    print(f"    ZIRLO cladding:       {CLAD_VOL_FRAC*100:.0f} vol%  @ {ZIRLO_DENSITY} g/cm³")
    ...
    json_path = os.path.join(OUTPUT_DIR, "uo2_all_results.json")
